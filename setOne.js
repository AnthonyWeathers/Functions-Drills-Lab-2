...
// CODE HERE
const func = (parameter1, parameter2) => `The first parameter is ${parameter1}. The second parameter is ${parameter2}`

// INVOKE THE FUNCTION HERE. THE PARAMETERS TAKE ANY DATATYPE.
let result = func(5, 'Universe')
console.log(result)
...
greeting("Tails", "Power", printName)
...
const pricesPlusTax = (prices, callback) => {
    for (price in prices) { // for (let i = 0; i < prices.length; i++)
        totalCost.push(prices[price] * 1.2)
    }
    callback(totalCost)
}
...
// CODE HERE
const newPriceArray = totalCost => {
    console.log(`The new array plus tax = ` + totalCost)
}
pricesPlusTax(prices, newPriceArray)
...
// CODE HERE
const multiplyingFactory = x => (y) => x >= 5 ? console.log(x * y) : console.log('Cannot multiply: the first number is smaller than 5.')
...
*/
const timesFour2 = multiplyingFactory(5)
timesFour2(4)