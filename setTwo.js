...
const uniq = (arr, callback) => {
  for (let i = 0; i < arr.length - 1; i++) { 
    //console.log(`Currently looking at index ${i} of array being ${arr[i]}`)
    for (let j = i + 1; j < arr.length; j++) {
      //console.log(`Inspecting index ${j} of array being ${arr[j]}`)
      if (arr[i] === arr[j]) { // [Tyler, ]
...
// CODE HERE
const addingFactory = number => (number2) => number + number2
/*
...
// CODE HERE
const addNUMBER = addingFactory(19)

console.log(addNUMBER(28))
...
const getUserById = (users, id, callback) => {
  ...
  for (person in users) {
    if (users[person].id === id) {
      callback(users[person])
    }
  }
}
